...
class PersonService {
     ...
     * @param {Object} person - Data for the new person entry
     */
    async createPerson(person, land ) {
        try {
            return await Person.create(person, land);
        } catch (err) {
            throw new CustomError(
                err.statusCode || 500,
                err.message || "Error creating person"
            );
        }
    }
